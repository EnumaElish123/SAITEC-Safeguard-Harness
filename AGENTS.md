...
- 2026.06.27 23:30 修复当前服务器推断代码链路：`side_branch_rules` 对 skipped 必需方法按中性结果参与聚合以避免缺失 progressive 崩溃，并为 S5 示例图片绝对路径增加仓库 `data/example-s5-images/` fallback；本地限制 `CUDA_VISIBLE_DEVICES=0` 跑通新增 `data/样例数据-合并.json` 共 27 条推断，四段 stage 均完成且 GPU 无残留占用；current-server 多模态 YAML 配置按本次提交要求未纳入推送。
